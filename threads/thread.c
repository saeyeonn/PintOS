...
update_mlfqs_priority(struct thread *t)
{
   ...
   int recent_cpu = round_fixed_point(fixed_point_div_int(t->recent_cpu, 4));
...
{
   if(t == idle_thread) return ;
   fixed_point coef = fixed_point_mult_int(load_avg, 2);
   coef = fixed_point_div(coef, fixed_point_add_int(coef, 1));
   t->recent_cpu = fixed_point_add_int(fixed_point_mult(coef, t->recent_cpu), t->nice);
}
...
void update_load_avg()
{
  ...
  load_avg = fixed_point_add(fixed_point_mult(fixed_point_mult_int(coef, 59), load_avg), 
                               fixed_point_mult_int(coef, ready_threads));
}
...
{
   return round_fixed_point(fixed_point_mult_int(thread_current()->recent_cpu, 100)); 
}
...
{
  return round_fixed_point(fixed_point_mult_int(load_avg, 100)); 
}
